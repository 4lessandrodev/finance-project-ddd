...
export interface CreateBenefitCalculationDto {
	userId: string;
	budgetBoxId: string;
	total: number;
}

export class CreateBenefitCalculationDomainService implements IDomainService<CreateBenefitCalculationDto, TransactionCalculationValueObject> {
	constructor (
		@Inject('BudgetBoxConnection')
		private readonly budgetBoxConnection: IBudgetBoxConnection
	){}
	async execute (data: CreateBenefitCalculationDto): Promise<TransactionCalculationValueObject> {
...
export default CreateBenefitCalculationDomainService;
